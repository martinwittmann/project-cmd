...
def fabric_get_file(self, remote, local=None, preserve_mode=True, callback=None):
    """
...
def fabric_put_file(self, local, remote=None, preserve_mode=True, callback=None):
    """
    ...
    if not remote:
        ...
        if local_base:
            ...
            if is_file_like:
                ...
                raise ValueError(
                    "Somehow got an empty local file basename ({!r}) when uploading to a directory ({!r})!".format(
                        # noqa
                        local_base, remote
                    )
                )
