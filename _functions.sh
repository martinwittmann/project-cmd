...
_project_update_php_env_for_tag() {
  ...
  _project_get_php_env_for_files "$project_tag" "${env_files[@]}" > "$output_file"
}
...
_project_get_php_env_for_files() {
  ...
  local function_name="__local_env"
  if [ -n "$tag" ]; then
    function_name="${function_name}_$tag"
  fi

  local output="<?php\n"
  output+="// Do not edit this file since it is generated. Run \"project run update_php_env\" to update it.\n"
